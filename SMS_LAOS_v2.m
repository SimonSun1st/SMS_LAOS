...
% + Elastic/viscous Lissajous decomposition
% + Decomposed curve CSV export
%
...
% No reuse, redistribution, modification, or commercial use is permitted
...
%   - no Excel saved
%   - result csv saved
%   - decomposed curve csv saved if save_curve_csv = true
%
% FULL MODE:
%   fast_mode = false
%   - save png plots
...
%
%   sigma_elastic =
%       gamma0 * [e1*T1(gamma/gamma0) + e3*T3(gamma/gamma0)]
%
%   sigma_viscous =
%       gamma0*omega * [v1*T1(gammadot/(omega*gamma0))
%                     + v3*T3(gammadot/(omega*gamma0))]
%
%   This version reconstructs the decomposed curves using first + third
%   Chebyshev terms, consistent with the SMS-LAOS parameters calculated here.
% =========================================================================
...
% ---- Curve CSV export ----
% This saves the total, elastic, viscous, reconstructed, and residual curves.
% Kept independent from fast_mode because these files are useful for OriginPro.
save_curve_csv = true;
...
curve_data_folder = fullfile(folder_path, 'SMS_LAOS_CurveCSV');
...
if ~exist(curve_data_folder, 'dir') && save_curve_csv
    mkdir(curve_data_folder);
end
...
fprintf('Save decomposed curve CSV: %d\n\n', save_curve_csv);
...
for k = 1:length(file_list)
    ...
    try
        ...
        % -------- Summary arrays --------
        summary_names{end+1} = base_name; %#ok<SAGROW>
        summary_freq(end+1) = out.freq_hz; %#ok<SAGROW>
        summary_I3I1(end+1) = out.I3_I1; %#ok<SAGROW>
        summary_e3e1(end+1) = out.e3_e1; %#ok<SAGROW>
        summary_v3v1(end+1) = out.v3_v1; %#ok<SAGROW>
        summary_S(end+1) = out.S; %#ok<SAGROW>
        summary_T(end+1) = out.T; %#ok<SAGROW>
        ...
        fprintf('  Reconstruction residual RMS = %.6g Pa, normalized = %.6g\n\n', ...
            out.residual_rms, out.residual_rms_over_I1);
        ...
        if save_selected_data_csv
            selected_table = table( ...
                ...
                out.strain, ...
                out.gammadot, ...
                out.gammadot_over_omega, ...
                out.stress, ...
                out.stress_elastic, ...
                out.stress_viscous, ...
                out.stress_reconstructed, ...
                out.stress_residual, ...
                'VariableNames', { ...
                'Time_s', ...
                'Strain', ...
        ...
        % -------- Save decomposed Lissajous curve data as CSV --------
        if save_curve_csv
            curve_table = table( ...
                out.time, ...
                out.strain, ...
                ...
                'Stress_Reconstructed_Pa', ...
                'Stress_Residual_Pa', ...
                'Chebyshev_x_StrainOverGamma0', ...
                'Chebyshev_y_StrainRateOverOmegaGamma0', ...
                'Total_Lissajous_X_Strain', ...
                'Total_Lissajous_Y_Stress_Pa', ...
                'Elastic_Lissajous_X_Strain', ...
                'Elastic_Lissajous_Y_StressElastic_Pa', ...
                'Viscous_Lissajous_X_StrainRateOverOmega', ...
                'Viscous_Lissajous_Y_StressViscous_Pa', ...
                'Reconstructed_Lissajous_X_Strain', ...
                'Reconstructed_Lissajous_Y_StressReconstructed_Pa'});

            writetable(curve_table, ...
                fullfile(curve_data_folder, [base_name '_LAOS_decomposed_curves.csv']));
        end
...
if ~isempty(summary_names) && (save_plots || save_fig_files)
    fig_sum = figure('Name', 'SMS-LAOS Summary', ...
...
if save_curve_csv
    fprintf('Decomposed curve CSV files saved in:\n%s\n', curve_data_folder);
end
...
function out = analyze_sms_laos(data, cycle_selection_mode, remove_first_cycles, remove_last_cycles, ...
    ...
    % -------- Chebyshev coefficients, keep user's convention --------
    e1 = Gp1;
    v1 = Gpp1 / omega;
    ...
    % -------- Elastic / viscous Lissajous decomposition --------
    % This uses the first + third Chebyshev terms calculated above.
    %
    % Elastic:
    ...
    %   y = strain_rate/(omega*gamma0)

    gammadot = gradient(strain, time);

    x_cheb = strain / gamma0;
    y_cheb = gammadot / (omega * gamma0);

    % Limit small numerical overshoot/noise outside [-1, 1].
    x_cheb = max(min(x_cheb, 1), -1);
    y_cheb = max(min(y_cheb, 1), -1);

    T1x = x_cheb;
    T3x = 4*x_cheb.^3 - 3*x_cheb;

    T1y = y_cheb;
    T3y = 4*y_cheb.^3 - 3*y_cheb;
    ...
    stress_residual = stress - stress_reconstructed;

    gammadot_over_omega = gammadot / omega;

    residual_rms = sqrt(mean(stress_residual.^2));

    if I1 ~= 0
        residual_rms_over_I1 = residual_rms / I1;
    else
        residual_rms_over_I1 = NaN;
    end
    ...
    out.gammadot = gammadot;
    out.gammadot_over_omega = gammadot_over_omega;

    out.x_cheb = x_cheb;
    out.y_cheb = y_cheb;

    out.stress_elastic = stress_elastic;
    out.stress_viscous = stress_viscous;
    out.stress_reconstructed = stress_reconstructed;
    out.stress_residual = stress_residual;

    out.residual_rms = residual_rms;
    out.residual_rms_over_I1 = residual_rms_over_I1;
...
function fig = plot_sms_laos_results(out, sample_name, cycle_selection_mode)
    ...
    xlabel('Strain')
    ylabel('Stress (Pa)')
    title('Total Stress-Strain Lissajous')
    grid on
    ...
    subplot(3,4,8)
    axis off
    text(0, 1.00, ['Sample: ' sample_name], 'FontSize', 9, 'Interpreter', 'none')
    text(0, 0.93, sprintf('Mode = %s', cycle_selection_mode), 'FontSize', 9, 'Interpreter', 'none')
    text(0, 0.86, sprintf('f = %.6f Hz', out.freq_hz), 'FontSize', 9)
    text(0, 0.79, sprintf('\\omega = %.6f rad/s', out.omega), 'FontSize', 9)
    text(0, 0.72, sprintf('Cycles used = %d', out.n_cycles_used), 'FontSize', 9)
    text(0, 0.65, sprintf('I1 = %.4g Pa', out.I1), 'FontSize', 9)
    text(0, 0.58, sprintf('I3 = %.4g Pa', out.I3), 'FontSize', 9)
    text(0, 0.51, sprintf('I3/I1 = %.4g', out.I3_I1), 'FontSize', 9)
    text(0, 0.44, sprintf('e1 = %.4g Pa, e3 = %.4g Pa', out.e1, out.e3), 'FontSize', 9)
    text(0, 0.37, sprintf('v1 = %.4g Pa.s, v3 = %.4g Pa.s', out.v1, out.v3), 'FontSize', 9)
    text(0, 0.30, sprintf('G''1 = %.4g Pa, G''''1 = %.4g Pa', out.Gp1, out.Gpp1), 'FontSize', 9)
    text(0, 0.23, sprintf('G''3 = %.4g Pa, G''''3 = %.4g Pa', out.Gp3, out.Gpp3), 'FontSize', 9)
    text(0, 0.16, sprintf('e3/e1 = %.4g, v3/v1 = %.4g', out.e3_e1, out.v3_v1), 'FontSize', 9)
    text(0, 0.09, sprintf('S = %.4g, T = %.4g', out.S, out.T), 'FontSize', 9)
    text(0, 0.02, sprintf('G_M'' = %.4g, G_L'' = %.4g', out.G_M_prime, out.G_L_prime), 'FontSize', 9)
    text(0, -0.05, sprintf('eta_M'' = %.4g, eta_L'' = %.4g', out.eta_M_prime, out.eta_L_prime), 'FontSize', 9)
    text(0, -0.12, sprintf('Residual RMS/I1 = %.4g', out.residual_rms_over_I1), 'FontSize', 9)

    subplot(3,4,9)
    plot(out.strain, out.stress_elastic, 'LineWidth', 1.2)
    xlabel('Strain')
    ylabel('$\sigma_{\mathrm{elastic}}$ (Pa)', 'Interpreter', 'latex')
    title('Elastic Lissajous')
    grid on

    subplot(3,4,10)
    plot(out.gammadot_over_omega, out.stress_viscous, 'LineWidth', 1.2)
    xlabel('$\dot{\gamma}/\omega$', 'Interpreter', 'latex')
    ylabel('$\sigma_{\mathrm{viscous}}$ (Pa)', 'Interpreter', 'latex')
    title('Viscous Lissajous')
    grid on

    subplot(3,4,11)
    plot(out.time, out.stress, 'LineWidth', 1.2); hold on
    plot(out.time, out.stress_reconstructed, '--', 'LineWidth', 1.2)
    xlabel('Time (s)')
    ylabel('Stress (Pa)')
    title('Total vs Reconstructed Stress')
    legend('Raw selected stress', 'Elastic + viscous', 'Location', 'best')
    grid on

    subplot(3,4,12)
    plot(out.strain, out.stress, 'LineWidth', 1.0); hold on
    plot(out.strain, out.stress_reconstructed, '--', 'LineWidth', 1.2)
    xlabel('Strain')
    ylabel('Stress (Pa)')
    title('Raw vs Reconstructed Lissajous')
    legend('Raw total', 'Reconstructed', 'Location', 'best')
    grid on
